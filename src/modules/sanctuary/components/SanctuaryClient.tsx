...
import { Palmtree, MapPin, Phone, Mail, Laptop, Waves, Sun, ShieldCheck, Leaf, Wind, Droplets, Heart, ChevronDown } from 'lucide-react';
...
export function SanctuaryClient({ initialRooms, settings }: SanctuaryClientProps) {
    ...
    // Safe fallback for amenities array
    ...
    return (
        <div className="min-h-screen bg-[#faf7f2] text-[#1c120c] flex flex-col justify-between scroll-smooth">
            <div>
                {/* Sticky Header */}
                <header className="sticky top-0 z-40 bg-[#1c120c]/95 backdrop-blur-md text-[#faf7f2] px-6 h-20 flex items-center justify-between border-b border-[#2b1d14] shadow-lg">
                    ...
                    <Link
                        href="/villas"
                        className="min-h-[44px] px-6 bg-[#c89349] text-[#1c120c] font-bold uppercase tracking-wider text-xs rounded-xl flex items-center justify-center hover:bg-[#b07d37] transition shadow-md"
                    >
                        {settings.reserve_button_text}
                    </Link>
                </header>

                {/* Dynamic Hero Header Banner with Seaview Logo/Icon */}
                <section className="bg-[#1c120c] text-[#faf7f2] py-20 px-6 text-center relative overflow-hidden">
            ...
            <span className="text-[11px] font-bold uppercase tracking-widest text-[#c89349] flex items-center justify-center gap-2">
              {settings.logo_url ? (
                    ...
                    <section className="max-w-7xl mx-auto px-6 py-12">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {amenitiesList.map((amenity, idx) => {
                                ...
                                return (
                                    <div key={idx} className="bg-white p-8 rounded-3xl border border-[#e6c898]/40 shadow-xs space-y-4">
                ...
                {galleryImages.length > 0 && (
                    <section className="bg-white border-t border-[#e6c898]/30 py-20">
                        <div className="max-w-7xl mx-auto px-6">
                            <div className="text-center mb-12 space-y-2">
                                <span className="text-[10px] font-bold uppercase tracking-widest text-[#c89349]">Visual Tour</span>
                                <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-[#1c120c]">The Sanctuary Gallery</h2>
            ...
            {/* Footer */}
            <footer className="bg-[#1c120c] text-[#faf7f2] relative overflow-hidden pt-16 pb-8 border-t border-[#2b1d14]">
